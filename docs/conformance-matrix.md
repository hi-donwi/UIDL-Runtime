...
| **Conformance Pass Rate** | **57 / 57** | **57 / 57** | **57 / 57** | compiler tests, not the shared suite |

Existing `action` cases remain schema-accept. Additive `action-exec` cases (2) execute `setState` write and `$data` reject.
